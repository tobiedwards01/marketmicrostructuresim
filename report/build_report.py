...
def build_cover(story):
    ...
    story.append(
        Paragraph(
            "A Limit-Order-Book Market Built From Scratch — Agent-Based Order Flow, "
...
def build_section_1(story):
    ...
    story.append(
        p(
            ...
            "interaction — bid-ask spreads, price impact, and adverse selection. It then replaces a "
            "naive market maker with an agent that learns its own quoting strategy through "
            "reinforcement learning, and checks what it learned against a closed-form analytical "
            "benchmark from the market-making literature.",
            "Lead",
        )
    )
    ...
    story.append(
        p(
            "This project targets finance and quant roles — trading, quantitative research, and "
            "market making specifically. It is a compressed version of what market-making and "
    ...
    story.append(
        bullets(
            [
                ...
                "<b>Technical communication</b> — this report, and the running design/decision log "
                "kept throughout the build (see Appendix A).",
            ]
        )
    )
    ...
    story.append(
        p(
            ...
            "structure — Sections 2 through 7 correspond directly to Phases 0/1 through 6 of the "
            "build; Section 8 draws conclusions across all of them."
        )
    )
    story.append(
        styled_table(
            ...
            [
                ["0 — Research & Design", "Core data model and event-loop design (DESIGN.md)", "2"],
                ["1 — Core Matching Engine", "Price-time-priority matching engine with a full test suite", "2"],
                ["2 — Baseline Agent Population", "Noise traders, an informed trader, a naive market maker", "3"],
                ["3 — Simulation & Metrics", "Event loop instrumentation and a metrics pipeline", "4"],
                ["4 — Emergent Behaviour Analysis", "Three stylized facts checked against theory", "5"],
                ["5 — Learning Market-Making Agent", "A trained RL agent vs. the Avellaneda-Stoikov benchmark", "6"],
                ["6 — Stress Test", "A flash-crash case study across three market-maker designs", "7"],
                ["7 — Writeup & Portfolio Report", "This document", "—"],
            ],
    ...
    story.append(
        callout(
            "Scope discipline",
            "Two things were deliberately cut and are flagged as such rather than silently skipped: "
            "a formal hypothesis-based property test suite (a randomized invariant test over 500 "
            "operations was judged to cover the same ground), and validation against real LOBSTER "
            "tick data (a separate, meaningfully sized piece of work in its own right — noted as the "
            "strongest candidate for extension in Section 8).",
        )
    )
...
def build_section_2(story):
    ...
    story.append(
        p(
            ...
            "DESIGN.md — the matching engine's data structures, the algorithm for walking the book, "
            "and the open design questions that needed resolving before implementation could begin. "
            "This section summarizes what was built and, more importantly, the specific design "
    ...
    story.append(
        p(
            ...
            "maker and taker order/agent ids, and the aggressor side — the side of the order that "
            "crossed the spread, which later turns out to be the single most useful field in the "
            "entire data model for downstream analysis."
        )
    )
    story.append(
        callout(
            "Why integer ticks, not floats",
            "Every price is stored as an integer number of ticks (one tick equals one cent) rather "
            "than a floating-point dollar amount. Comparing floats for price-level equality is a "
            "classic source of matching-engine bugs (0.1 plus 0.2 does not equal 0.3 in binary "
            "floating point); representing price as an integer from the start removes an entire "
            "category of bug before it can occur. Conversion to a dollar figure happens only at the "
            "display boundary.",
        )
    )
    ...
    story.append(
        p(
            ...
            "queue of resting orders at that price, implemented with the sortedcontainers library's "
            "SortedDict for O(log n) price-level insertion and O(1) best-price lookup. A separate "
            "dictionary maps order id to order object for O(1) cancellation lookups. Best bid, best "
            "ask, spread, and mid price are all read properties computed from the book's current "
            "state."
        )
    )
    ...
    story.append(
        p(
            ...
            "predicate holds (always true for market orders; for limit orders, while the best "
            "opposing price still crosses the incoming order's own limit price). At each price "
            "level it fills against the oldest eligible resting order first — price-time priority — "
            "decrementing both sides' remaining quantity and emitting a Trade. A resting order that "
            "reaches zero remaining quantity is marked filled and removed from its queue; an "
            "incoming order with quantity left over after the book is exhausted either rests (limit "
            "orders) or has its remainder cancelled immediately (market orders — standard "
            "immediate-or-cancel semantics, since a resting market order is not a concept that "
            "exists on any real exchange)."
        )
    )
    ...
    story.append(
        bullets(
            [
                ...
                "place rather than removed from its queue immediately; the matching loop skips past "
                "cancelled orders (and purges them) when it next reaches them. Simpler to implement "
                "correctly than an eagerly-maintained doubly-linked list, at the cost of needing to "
                "explicitly account for cancelled-but-not-yet-purged orders anywhere book state is "
                "read — which surfaced as a real, if minor, correctness bug later (see the callout "
                "below).",
                ...
                "past that resting order (continuing to the next order at that price level, or the "
                "next price level) rather than cancelling either side. This is more forgiving than "
                "most real exchanges, which typically cancel one or both sides outright — a "
                "deliberate simplification, documented as such, and one whose consequences (a book "
                "that can briefly report a \"locked\" price from one agent's own crossed quotes) "
                "were later traced directly to a real, observable artifact in the simulator's output "
                "(Section 3).",
                "<b>Market orders never rest.</b> Immediate-or-cancel semantics: fill whatever is "
                "available, cancel the rest instantly. This also produces a useful signal on its own "
                "— the fraction of a market order that goes unfilled during a liquidity shortage is "
                "exactly what the Phase 6 stress test wants to observe.",
            ]
        )
    )
    story.append(
        callout(
            ...
            "tradeable actually resting there — a cancelled order sitting at the front of the queue. "
            "This was not a hypothetical concern: it was caught in Phase 2 when the naive market "
            "maker cancelled its own stale quote and immediately asked for the current mid price to "
            "decide where to requote, and got a stale answer back. Fixed by having best-bid/best-ask "
            "actively purge cancelled orders from the front of the queue before reporting a price — "
            "a small, well-contained fix once the lazy-deletion contract was made to apply "
            "consistently everywhere the book state is read, not only inside the matching loop "
            "itself.",
        )
    )
    ...
    story.append(
        p(
            "The simulator is a discrete-event simulation: a min-heap of (timestamp, sequence "
            "number, agent id) tuples, popped in time order. The sequence number is a single shared "
            "counter used both to break timestamp ties in the event queue and as every order's own "
            "sequence field — the same counter serving two purposes so that \"what happened at "
            "exactly this simulated instant, and in what order\" is always fully deterministic and "
            "reproducible, which matters a great deal once simulation runs are being compared to "
            "each other (Phase 4) or replayed for a stress-test scenario (Phase 6)."
        )
    )
    ...
    story.append(
        p(
            ...
            "cases, and — as a final hardening pass — a 500-step randomized test that submits a "
            "mixed sequence of orders and cancellations across five agents and asserts, after every "
            "single step, that no two different agents' resting orders could have crossed without "
            "matching, plus a global conservation check (total filled quantity across all orders "
            "equals twice the total traded volume) at the end."
        )
    )
...
def build_section_3(story):
    ...
    story.append(
        p(
            ...
            "trade in the same market, then each agent in turn."
        )
    )
    story.extend(h2("Shared Architecture"))
    story.append(
        p(
            "Every agent implements two methods: <b>next_wake_time</b>, which tells the event loop "
            "when it should next be woken (agents typically use a Poisson arrival process — an "
            "exponentially distributed inter-arrival time — for realistic, bursty order flow), and "
            "<b>act</b>, which is called when it wakes and may submit or cancel orders through a "
            "narrow <b>MarketAccess</b> interface (submit a limit order, submit a market order, "
            "cancel an order, and read the current book state). Each agent owns its own seeded "
            "random-number generator, so an entire simulation run is exactly reproducible given a "
            "fixed set of per-agent seeds — every result in this report was produced this way, and "
            "every number quoted below was reproduced from a fresh run immediately before this "
            "report was written."
        )
    )
    story.append(
        p(
            "The three market-making agents (naive, Avellaneda-Stoikov, and the learned agent) share "
            "a further common base, <b>QuotingAgent</b>, introduced midway through the project once "
            "it became clear the same machinery — cancel the previous two-sided quote, reconcile "
            "fills and cash against the shared trade log, respect an inventory limit — was about to "
            "be duplicated a third time. Each concrete market maker now implements only the pricing "
            "decision itself; the base class owns everything else, including tracking each agent's "
            "own running cash and inventory from the shared trade log — not just from the immediate "
            "return value of its own order submissions, since a resting quote can be filled by "
            "another agent at any later wake, with no way for the agent to know about it except by "
            "checking the trade log the next time it wakes."
        )
    )
    ...
    story.append(
        p(
            ...
            "wake, it submits one random limit order — side chosen uniformly, price a random offset "
            "from the current live mid price (falling back to a fixed reference price only when the "
            "book is empty), quantity drawn from a configured range — and cancels its own previous "
            "order first, so it holds exactly one live order at a time, replaced every period. It "
            "has no view of value beyond \"wherever the market currently is.\" Five independent "
            "noise traders (different random seeds) form the baseline liquidity and order-flow "
            "generator for every simulation in this project."
        )
    )
    story.append(
        callout(
            ...
            "followed once the metrics pipeline (Phase 3) could actually chart them: order-book "
            ...
            "could end up priced on the wrong side of a trader's own new order, producing a "
            "genuinely negative bid-ask spread when self-trade prevention correctly refused to match "
            "the two (see the self-trade-prevention decision in Section 2). Matching the actual "
            "textbook zero-intelligence model — one live order per trader — fixed both artifacts at "
            "once.",
        )
    )
    ...
    story.append(
        p(
            ...
            "aggressively with a market order; far enough below the best bid, it sells. It trades "
            "using market orders specifically because an informed trader with a real, if temporary, "
            "information edge wants immediate execution before that edge decays — not to post "
            "passive liquidity and wait. This is the single mechanism in the whole simulator "
            "responsible for adverse selection: it systematically picks off quotes that have gone "
            "stale relative to where the market maker's beliefs should have moved."
        )
    )
    ...
    story.append(
        p(
            ...
            "its inventory limit on that side is reached. No skewing, no reaction to time or "
            "volatility — the simplest possible market-making strategy, and the baseline every later "
    ...
    story.append(
        callout(
            ...
            "Because the inventory check only gates whether to submit a new quote — not how large "
            "to size it — a single full-size fill landing exactly at the boundary can carry "
            "inventory a little past the configured cap. Observed directly in a real run: a "
            "configured 50-unit cap, a final inventory of 51 in one Phase 2 run and +57 in the "
            "Phase 6 stress test. Left as-is deliberately, since it is a genuine, useful finding "
            "about naive strategies rather than a bug to silently patch — see Section 7.",
        )
    )
    ...
    story.append(
        p(
            "where gamma is the market maker's risk aversion, sigma is the mid price's volatility, "
            "time remaining is the distance to a fixed horizon, and k controls how quickly order-flow "
            "intensity decays with distance from the mid price. Two behaviours fall directly out of "
            "the formulas that the naive strategy cannot produce: the reservation price shifts away "
            "from the mid price in the direction that reduces inventory — a long position pushes "
            "both quotes down, a short position pushes them up — and the spread widens with more "
            "time remaining to the horizon (more time for inventory risk to hurt) and narrows as "
            "the horizon approaches. Sigma is estimated from realized data (a real simulated run's "
            "mid-price volatility) rather than guessed; gamma and k are hand-tuned so the resulting "
            "spread lands in a realistic range for this market, since there is no equally direct way "
            "to estimate either from data — a documented simplification, not an oversight."
        )
    )
    ...
    story.append(
        p(
            "A tabular Q-learning agent — deliberately simple, per the project's own guidance to "
            "start with tabular methods and reach for deep reinforcement learning only if the state "
            "space genuinely needs it, which a fifteen-state, twenty-five-action problem does not. "
            "Full design detail is in Section 6; in brief, its state is a discretized (inventory "
            "bucket, time-remaining bucket) pair, its action is a discretized (spread choice, skew "
            "choice) pair deliberately mirroring Avellaneda-Stoikov's own two decision variables, "
            "and its reward at each decision is the change in its own mark-to-market profit and "
            "loss since the previous decision — so inventory risk becomes a real, experienced cost "
            "rather than a hand-tuned penalty term bolted on separately."
        )
    )
...
def build_section_4(story):
    ...
    story.append(
        p(
            ...
            "trading — no separate logging pass or after-the-fact reconstruction required."
        )
    )
    ...
    story.append(
        bullets(
            [
                ...
                "both trade-driven impact (a fill sweeping through resting liquidity) and "
                "quote-driven impact (a new best price posted without necessarily trading), since "
                "both genuinely move a real market.",
                ...
                "gap flagged in Section 2) — a naive sum over the raw queues would silently "
    ...
    story.append(
        p(
            "A single simulated session — five noise traders, one informed trader, one naive market "
            "maker, run for two hundred simulated time units — produces the four-panel dashboard "
            "below: spread over time, order book depth over time, price impact against trade size, "
            "and mark-to-market profit and loss per agent."
        )
    )
    ...
    story.append(
        callout(
            ...
            "Every one of the five noise traders loses money; the informed trader and the market "
            "maker both profit. This is the textbook adverse-selection story appearing directly in "
            "the P&amp;L panel with no further analysis needed — informed order flow and disciplined "
            "market-making both extract value from uninformed liquidity, exactly as Glosten-Milgrom "
            "and Kyle both predict.",
        )
    )
...
def build_section_5(story):
    ...
    story.append(
        p(
            ...
            "informed trader's arrival rate was swept across six levels (0.05 through 2.0), with "
    ...
    story.append(
        p(
            ...
            "wider spread across a forty-times range in informed intensity — but monotonic and "
            ...
            "adverse selection (Avellaneda-Stoikov, Section 6) would be expected to show a "
    ...
    story.append(
        p(
            ...
            "outcome of an informed trader optimally hiding order flow in noise — the textbook "
            "baseline. A large body of later empirical and theoretical work (the market-impact "
            "\"square-root law\" among others) instead finds impact that grows sub-linearly — "
    ...
    story.append(
        p(
            "<b>Result: broadly concave, but noisy.</b> Concavity was checked as whether impact "
            "per unit of size falls as size grows — if so, marginal impact is shrinking, the "
            "definition of concavity. The correlation between size and impact-per-unit-size was "
            "−0.767, consistent with concavity, and the chart shows observed impact sitting well "
            "below the linear Kyle reference across the whole size range, never catching up as "
            "size grows. One bucket (size five) is a visible non-monotonic dip, and per-bucket "
            "sample sizes range from thirty-three to a hundred — enough to support the qualitative "
            "claim (sub-linear, not linear) but not a precise functional form."
        )
    )
    ...
    story.append(
        p(
            ...
            "real asset returns: large price moves tend to be followed by more large moves, of "
            "either sign, and small moves by small ones. This shows up as clearly positive "
    ...
    story.append(
        p(
            "<b>Result: partial support, not a clean match.</b> There is a real, positive lag-one "
            "signal in absolute returns (0.165) — some short-lived clustering is genuinely present "
            "— but it decays to essentially zero by lag two and stays there; real markets typically "
            "show this clustering persisting across dozens or hundreds of lags. This simulator does "
            "not reproduce the strong, persistent form of the stylized fact, and the reason is "
            "mechanistic rather than a shortfall of data: nothing in the simulator has time-varying "
            "volatility built into it. The informed trader's true-price process uses a constant "
            "volatility parameter for the whole run — there is no mechanism for quiet and turbulent "
            "periods to alternate. Also worth noting: raw returns show a negative, not near-zero, "
            "autocorrelation at short lags — a mean-reversion signature most likely coming from the "
            "market maker actively pulling price back toward its own quotes on every requote, and "
            "from noise traders re-centring on the live mid price each time they wake."
        )
    )
    ...
    story.append(
        p(
            "Two of three stylized facts hold up cleanly; the third holds up partially, with a "
            "specific, mechanistic explanation rather than an unexplained gap — a more useful result "
            "for a technical interview than three clean checkmarks, since it demonstrates the "
            "simulator was actually interrogated rather than declared successful on the first pass."
        )
    )
    story.append(
        callout(
            "What wasn't done",
            "The brief's optional stretch goal — sanity-checking against real LOBSTER tick data — "
            "was not attempted. It requires acquiring and licensing-checking an external dataset and "
            "choosing a comparable instrument and time window: a meaningfully separate piece of "
            "work in its own right, not an extension of what's here. See Section 8 for why it's the "
            "strongest candidate for future work.",
        )
    )
...
def build_section_6(story):
    ...
    story.append(
        p(
            "The naive market maker — fixed spread, no inventory skewing — is replaced here with two "
            "things: a closed-form analytical benchmark (Avellaneda-Stoikov, already introduced in "
            "Section 3) and a tabular Q-learning agent trained purely on a profit-and-loss reward "
            "signal, with no knowledge of the Avellaneda-Stoikov formula at all. The central "
            "question this phase answers: does an agent that only ever observes whether its own "
            "mark-to-market profit and loss went up or down rediscover anything resembling the "
            "textbook-optimal quoting behaviour?"
        )
    )
    ...
    story.append(
        p(
            "State is a discretized pair: an inventory bucket (five buckets, spanning very short to "
            "very long relative to the agent's inventory limit) and a time-remaining bucket (three "
            "buckets: early, mid, late in the episode) — fifteen states in total. Action is a "
            "discretized pair too: a half-spread choice from five values and a skew choice from five "
            "values — twenty-five actions — deliberately built to mirror Avellaneda-Stoikov's own "
            "two decision variables, spread driven by time-to-horizon and skew driven by inventory, "
            "so the eventual comparison between what the agent learns and the closed form is "
            "genuinely apples-to-apples rather than comparing unrelated representations. Reward at "
            "each decision is the change in the agent's own mark-to-market profit and loss since the "
            "previous decision, so inventory risk is a real, experienced cost the agent has to learn "
            "to manage — not a hand-tuned penalty term layered on separately."
        )
    )
    story.append(
        p(
            "Training used a standard off-policy Q-learning update (the maximum over the next "
            "state's action values, not the actually-chosen next action) with a constant learning "
            "rate, epsilon-greedy exploration decaying from 0.3 to a floor of 0.02 over training, "
            "and ties among equally-good actions broken randomly rather than always toward the "
            "first index. Tabular Q-learning was chosen deliberately over a deep-learning approach, "
            "following the project's own guidance to start simple and reach for a neural network "
            "only if the state space genuinely needs it — fifteen states and twenty-five actions do "
            "not."
        )
    )
    ...
    story.append(
        p(
            "Trained for 1,200 episodes against the same order flow — five noise traders, one "
            "informed trader — that every other market-making agent in this project has faced. "
            "Performance converged to a stable average mark-to-market profit and loss of "
            "roughly $290 to $310 per two-hundred-time-unit episode within about fifty episodes, "
            "and held there for the remaining eleven hundred with no divergence or collapse."
        )
    )
    ...
    story.append(
        p(
            ...
            "relationship that looked backwards from what theory predicts — quotes barely moving "
            "with inventory, and what little movement there was pointed the wrong way. Before "
            "writing that up as \"the agent learned something genuinely different from theory,\" a "
            "per-state visit-count tracker was added to check whether it was simply an "
            "under-exploration artifact — extreme-inventory states, reached less often under a "
            "policy that is actively trying to stay near flat, seemed like the obvious suspect. The "
            "counts refuted that theory directly: all fifteen states received between twenty-three "
            "thousand and forty-six thousand visits across six hundred episodes, roughly uniform, "
    ...
    story.append(
        p(
            ...
            "explanation: Q-learning here uses a constant learning rate rather than a decaying one, "
            "so the table never fully settles to a fixed point — aggregate performance had already "
            "plateaued by episode one hundred, but the fine-grained, genuinely low-signal "
            "distinction of exactly which skew choice is marginally better than its neighbours kept "
            "needing more updates to resolve cleanly. This is reported here, rather than quietly "
            "presenting only the corrected second run, because \"a first hypothesis turned out to "
            "be wrong, here is what the cause actually was\" is a more honest and more useful "
            "account of the process than a single clean result would be."
        )
    )
    ...
    story.append(
        p(
            "Both narrow as the horizon approaches — the learned agent in one discrete step (its "
            "coarse three-bucket time discretization), Avellaneda-Stoikov smoothly and "
            "continuously. This is a genuine result: the learned agent was never told that spread "
            "should shrink near the end of an episode — it only ever observed a profit-and-loss "
            "number, and rediscovered the direction of the closed-form prediction from reward "
    ...
    story.append(
        p(
            "Direction matches on skew too — both push quotes down when long, up when short — but "
            "magnitude diverges sharply: Avellaneda-Stoikov's skew at maximum inventory is roughly "
            "a hundred and forty times larger than the learned agent's. This is not really \"the "
            "learned agent failed to discover what theory says is correct.\" Avellaneda-Stoikov's "
            "skew term is linear and mathematically unbounded in inventory: at fifty units of "
            "inventory in this simulated market it prescribes moving quotes by roughly seven "
            "dollars on a hundred-dollar instrument — not a quote anyone would actually post, since "
            "at that point it is not making a market so much as refusing to trade. This is a "
            "well-documented practical critique of the raw Avellaneda-Stoikov formula; real trading "
            "desks typically cap or dampen the inventory term. The learned agent, constrained to a "
            "small, human-designed action grid, never explores anything close to that scale — "
            "arguably a more realistic constraint than the closed form imposes on itself."
        )
    )
    story.append(
        callout(
            ...
            "did not rediscover the closed form's specific magnitudes for inventory skew — where "
            "the closed form's own unbounded linear term is arguably the less realistic of the two "
            "once real quote sizes are considered.",
        )
    )
...
def build_section_7(story):
    ...
    story.append(
        p(
            ...
            "the true price partway through an otherwise ordinary session — simulating unexpected "
            "bad news — and a look at how each of the three market makers built across this project "
            "holds up against it."
        )
    )
    ...
    story.append(
        p(
            ...
            "the informed trader's belief is knocked down by fifteen dollars — simulating "
            "unexpected bad news — implemented by calling the event loop's run-until method twice "
            "in succession with the perturbation applied directly to the agent between the two "
            "calls. No new simulator machinery was required for this: it is pure orchestration of "
            "what Phases 1 through 5 already built. Everything downstream of the perturbation — the "
            "resulting burst of aggressive informed selling as its signal diverges from the "
            "market's still-stale quotes, and how far the price actually moves — is genuine "
    ...
    story.append(
        callout(
            ...
            "simulated market, and every other agent reacts to that market's own live mid price — "
            "so a different market maker genuinely produces a different emergent market, not simply "
            "a different profit-and-loss outcome layered on an identical price path. That is "
            "realistic — a real market maker's own liquidity provision does shape how a shock "
            "propagates — but it means any claim about why one market's outcome differs from "
            "another's is offered here as a plausible mechanism, not an isolated, statistically "
            "proven cause.",
        )
    )
    story.append(
    ...
    story.append(
        p(
            ...
            "seven units past its own configured cap, via the same limit-checks-whether-not-how-much "
            "overshoot mechanism first noted in Phase 2. It also finished the most profitable of the "
            "three in this specific run — closer to luck than to skill, since it accumulated a large "
            "long position at falling prices during the crash and the price happened to recover by "
            "the end of the run. A single run's profit outcome for an inventory-blind strategy "
            "should not be read as evidence that it handled the shock well."
        )
    )
    ...
    story.append(
        p(
            ...
            "mechanism: its reservation price re-centres on the live mid price every requote, and "
            "during the crash the mid itself was gapping violently, so each new quote chased an "
            "already-moving target rather than anchoring it — and, per the caveat above, that "
            ...
            "its own, actually worked — it stayed close to flat throughout, minus two to minus "
            "eleven units — but that discipline did not translate into good financial outcomes here, "
            "undercutting the theory's implicit promise that managing inventory well is sufficient."
        )
    )
    ...
    story.append(
        p(
            ...
            "$0.60 — more than the naive strategy, nowhere close to Avellaneda-Stoikov's spike; it "
            ...
            "Avellaneda-Stoikov formula, a crash-detection rule, or any explicit notion that this was "
            "a stress scenario at all — it learned a policy against ordinary order flow in Phase 5 "
            "that happened to generalize reasonably well to a scenario it had never seen."
        )
    )
...
def build_section_8(story):
    ...
    story.append(
        p(
            "Two distinct mechanisms, and it matters which one a given market maker actually has. "
            "First, order-flow composition: spread widens with informed-trading intensity even for "
            "a market maker with no explicit adverse-selection pricing at all (Section 5), because "
            "faster quote depletion under heavier informed flow leaves a wider effective spread "
            "while the requote cycle catches up — a second-order, mechanical effect. Second, and far "
            "more powerful when present: an explicit pricing model. Avellaneda-Stoikov's spread "
            "responds directly to time-to-horizon and a calibrated volatility parameter, and — in "
            "the stress test — its market showed a spread more than ten times wider than baseline "
            "during genuine turbulence, something no fixed-spread strategy could ever produce. The "
            "practical lesson: a market maker's spread is only as responsive as the model that sets "
            "it. A fixed-spread strategy is not \"conservative\" in any meaningful sense during a "
            "shock — it is simply blind to it."
        )
    )
    ...
    story.append(
        styled_table(
            ...
            [
                ...
                ["Avellaneda-Stoikov", "Model risk: a formula calibrated on calm-market volatility, with no adverse-selection term of its own, degrades badly exactly when the market conditions it assumed stop holding — good textbook inventory discipline did not prevent the worst P&L outcome under stress."],
                ["Q-learning", "Generalization risk: it performed well in a stress scenario it was never trained on, but that is a favourable roll of a single run's random draw, not a guarantee — and its action grid is coarse enough (five spread choices, five skew choices) that it may be structurally unable to react as aggressively as a genuinely severe shock would require."],
            ],
    ...
    story.append(
        bullets(
            [
                ...
                "than calibrating it once from a calm baseline — the single clearest lever for "
                "closing the gap between its theoretical promise and its stress-test performance.",
                ...
                "<b>Give the informed trader's true-price process time-varying volatility</b> — a "
                "regime-switching or GARCH-style process instead of a constant one — which the "
                "Phase 4 volatility-clustering result suggests is the single missing mechanism "
                "standing between this simulator and a much more realistic return-series signature.",
                ...
                "<b>Run the stress test across multiple random seeds</b>, not one illustrative run — "
                "the naive strategy's positive profit and loss in Section 7 looks like it could "
                "easily flip sign under a shock that does not recover by the end of the simulated "
                "session.",
            ]
        )
    )
    ...
    story.append(
        p(
            "Every phase of this project produced a runnable artifact, not just a design document: a "
            "matching engine with a hundred and thirty-nine automated tests, a metrics dashboard "
            "generated from a real simulated session, three theory comparisons with numbers "
            "reproduced from a fresh run for this report, a trained reinforcement-learning agent "
            "committed to the repository as an actual model file, and a stress test whose "
            "conclusions were checked against real output rather than assumed. Several of the most "
            "useful findings in this report — the negative-spread artifact traced to self-trade "
            "prevention, the false-alarm-then-fix story in the reinforcement-learning training, "
            "Avellaneda-Stoikov's poor stress-test showing despite good inventory discipline — were "
            "not anticipated going in. They surfaced because the project was built to be interrogated "
            "at every stage, not just demonstrated once at the end."
        )
    )
    ...
    story.append(
        bullets(
            [
                ...
                "sortedcontainers — the order book's price-level structure",
                "matplotlib — every chart in this report",
                "pytest / pytest-cov — the 139-test suite",
                "reportlab — this report itself",
            ],
            style="Bullet",
        )
    )
